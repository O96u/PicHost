...
- 图库批量打标：管理员/跨归属场景下标签解析与失败提示
- 上传鉴权：`Auth-Token` 与表单 `token` 优先于会话；兼容全局 `API_UPLOAD_TOKEN`（上传归属首位管理员）

### 改进

- **API 页**：每账号单一 API Token，界面与文档统一
- **上传 API**：仅支持 `tagNames`（按名创建或复用标签）；Twikoo 兼容路由同步
- **标签管理**：按图片数/最近使用排序；桌面列布局；窄屏卡片列表
- **API 文档**：参数表列宽，避免 form-data 列异常换行
